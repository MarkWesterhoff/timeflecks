...
public class TaskComponent extends JComponent
{
	...
	public void paintComponent(Graphics g)
	{
		...
		// The frame will get updated each time when we do this, so we can just go off of that. 	
		...
		int fontHeight = (int)g2.getFont().getLineMetrics(task.getName(), frc).getHeight();
		...
		// TODO  This should be changed to draw components within the bounds of the component and that's it and not require knowledge of its frame, and then it will be given a place to draw by the calendar.
		
		// Note that it is our job not to draw outside of our insets...
		g2.drawString(task.getName(), frame.x + getInsets().left + leftInset, frame.y + getInsets().top + topInset);	
	}
	...
	public static void main(String[] args)
	{
		try
		{
			...
			TaskComponent p = new TaskComponent(t, new Rectangle(10, 10, 200, 100));
