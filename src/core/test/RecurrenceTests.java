...
public class RecurrenceTests
{
	...
	{
		int currentTaskNum = Timeflecks.getSharedApplication().getTaskList()
				.getTasks().size();
		...
		GlobalLogger.getLogger().logp(Level.INFO, "test.core.RecurrenceTests",
				"testRecurrence()",
		...
		{
			Timeflecks.getSharedApplication().getTaskList().addTask(ta);
		}
		assertEquals("Recurrence should add 15 new tasks for weekly",
				Timeflecks.getSharedApplication().getTaskList().getTasks()
						.size(), 15 + currentTaskNum);
		currentTaskNum += 15;
		assertTrue(
				"Last date should be less than recurrence date",
				Timeflecks.getSharedApplication().getTaskList().getTasks()
						.get(currentTaskNum - 1).getDueDate()
						.compareTo(c.getTime()) < 1);
