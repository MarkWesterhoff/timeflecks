...
public class SQLiteConnectorTest
{
			...
			ClassNotFoundException
	{
		...
		// Positive tests
		new SQLiteConnector(files.get(0), true).serializeAndSave(new Task(
				"task1"));
		new SQLiteConnector(files.get(1), true).serializeAndSave(new Task(
				"task2"));
	 ...
	 */
	@Test
	public void testSaveAndLoad() throws SQLException, IOException,
			ClassNotFoundException
	{
		// Make new clean database
		new File("testSaveAndLoad.db").delete();
		Timeflecks.getSharedApplication().setDBConnector(
				new SQLiteConnector(new File("testSaveAndLoad.db"), true));
		...
		Task returnTask = Timeflecks.getSharedApplication().getDBConnector()
				.getSerializedTask(task.getId());
		...
		Event returnEvent = (Event) Timeflecks.getSharedApplication()
				.getDBConnector().getSerializedEvent(event.getId());
			...
			ClassNotFoundException
	{
		...
		Timeflecks.getSharedApplication().setDBConnector(
				new SQLiteConnector(new File("testDelete.db"), true));
	...
	public void testGetHighestId() throws SQLException, IOException
	{
		...
		Timeflecks.getSharedApplication().setDBConnector(
				new SQLiteConnector(new File("testGetHighestId.db"), true));

		Timeflecks.getSharedApplication().getDBConnector()
				.serializeAndSave(new Task("t1a"));
		Timeflecks.getSharedApplication().getDBConnector()
				.serializeAndSave(new Task("t1b"));
		Timeflecks.getSharedApplication().getDBConnector()
				.serializeAndSave(new Task("t1c"));
		Timeflecks.getSharedApplication().getDBConnector()
				.serializeAndSave(new Event("e1a", new Date(), new Date()));

		Task t2 = new Task("t2");
		Timeflecks.getSharedApplication().getDBConnector().serializeAndSave(t2);

		long maxId = Timeflecks.getSharedApplication().getDBConnector()
				.getHighestID();
			...
			ClassNotFoundException
	{
		...
		for (int i = 0; i < numTasks; ++i)
		{
			...
			Event e = new Event("event" + Integer.toString(i), new Date(),
					new Date());
		...
		Timeflecks.getSharedApplication().getDBConnector().getAllTasks();
		Timeflecks.getSharedApplication().getDBConnector().getAllEvents();
